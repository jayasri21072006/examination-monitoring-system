...
def update_frames(feed_sources: list[str]) -> None:
    for src in feed_sources:
        frame = read_feed_frame(src)

        # Only update if a real frame is returned
        if frame is not None:
            st.session_state.feed_frames[src] = frame
            st.session_state.feed_status[src] = "Connected"
...
def start_background_camera():
    ...
    # 🔥 FORCE OPEN ALL CAMERAS IMMEDIATELY (turn LED on)
    for src in st.session_state.feed_list:
        init_feed_state(src)
        frame = read_feed_frame(src)
        if frame is not None:
            st.session_state.feed_frames[src] = frame
...
def main() -> None:
    ...
    # ALWAYS refresh to keep camera live
   # Faster refresh for immediate camera display
    time.sleep(0.03 if st.session_state.running else 0.05)
    st.rerun()
